...
describe('messageActions', () => {
    it('undoes a state change', () => {
        messageActions.undo();
        expect(sendMessage).lastCalledWith({type: MessageType.UNDO});
    });

    it('redoes a state change', () => {
        messageActions.redo();
        expect(sendMessage).lastCalledWith({type: MessageType.REDO});
    });

    describe('feature', () => {
        it('adds a feature below', () => {
            messageActions.featureDiagram.feature.addBelow('FeatureIDE');
            expect(sendMessage).lastCalledWith({type: MessageType.FEATURE_DIAGRAM_FEATURE_ADD_BELOW, belowFeature: 'FeatureIDE'});
        });

        it('removes a feature', () => {
            messageActions.featureDiagram.feature.remove(['FeatureIDE']);
            expect(sendMessage).lastCalledWith({type: MessageType.FEATURE_DIAGRAM_FEATURE_REMOVE, feature: 'FeatureIDE'});
        });
    ...
    describe('features', () => {
        it('adds a feature above', () => {
            ...
            expect(sendMessage).lastCalledWith({type: MessageType.FEATURE_DIAGRAM_FEATURE_ADD_ABOVE, aboveFeatures: ['FeatureIDE', 'Eclipse']});
        });
    });
});